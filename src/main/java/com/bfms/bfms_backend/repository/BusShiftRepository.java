...
public interface BusShiftRepository extends JpaRepository<BusShift, Integer> {
    ...
    // 2. Tìm các ca chạy dưới dạng Projection để tối ưu hiệu năng (Không cần JOIN FETCH thủ công vì Projection sẽ xử lý)
    @Query("SELECT b FROM BusShift b WHERE b.node.route.id = :routeId AND b.status = :status")
    List<BusShiftProjection> findActiveShiftsProjectedBy(@Param("routeId") Integer routeId, @Param("status") ShiftStatus status);
